...
rule filter_calls:
    ...
    filter out no_assertion and no_interpretation
    '''
    input:
        vcf = clinvar_vcf,
    output:
    ...
    shell:
        r'''
        bcftools view -v "snps,indels"  -e 'CLNREVSTAT~"no_assertion" | CLNREVSTAT~"no_interpretation" | (CLNSIG!~"benign/i" & CLNSIG!~"pathogenic/i") | CLNSIG~"Conflicting"' {input.vcf} -Oz -o {output.vcf}
        tabix -f {output.vcf}
        '''
...
rule extract_data:
    ...
    shell:
        r'''
        bcftools query -i '(UTR3!=".")&&(ALT!=".")' -f "%CHROM\t%POS\tGT=%REF/%ALT;seq_name=%UTR3\t%CLNSIG\n" {input.vcf}|grep 'athogenic'|awk 'BEGIN{{FS="\t";OFS="\t"}}{{print $1,$2-1,$2,$3}}' > {output.tsv}
        '''
