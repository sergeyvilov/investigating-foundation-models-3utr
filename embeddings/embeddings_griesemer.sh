...
for model_name in 'dnabert' 'dnabert-3utr-2e' 'dnabert2' 'dnabert2-zoo' 'dnabert2-3utr-2e' 'ntrans-v2-100m' 'ntrans-v2-100m-3utr-2e'; do
    ...
    if [[ $model_name =~ "-3utr" ]]; then
       fasta=$data_dir'/mpra/griesemer_2021/fasta/variants_rna.fa'
    else
       fasta=$data_dir'/mpra/griesemer_2021/fasta/variants_dna_fwd.fa'
    fi

    if [[ $model_name = "dnabert" ]] || [[ $model_name = "dnabert-3utr-2e" ]]; then
       crop_options="--max_seq_len 508 --crop_center"
    else
       crop_options=""
    fi
    ...
    output_dir="$data_dir/mpra/griesemer_2021/embeddings/$model_name/"
    mkdir -p $output_dir
    srun -p gpu_p --qos=gpu_normal $constraint -o ${output_dir}/log.o -e ${output_dir}/log.e --nice=10000 -J $model_name-gries -c 4 --mem=64G --gres=gpu:1 --time=1-00:00:00 \
