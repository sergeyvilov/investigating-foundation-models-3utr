...
for model_name in 'dnabert' 'dnabert2' 'ntrans-v2-500m' 'ntrans-v2-250m' 'dnabert-3utr' 'dnabert2-3utr' 'ntrans-v2-250m-3utr'; do
    ...
    if [[ $model_name =~ "-3utr" ]]; then
       ...
       fasta=$data_dir'/variants/selected/variants_dna_fwd_1024bp.fa'
    fi
    output_dir="$data_dir/variants/embeddings/$model_name/"
    mkdir -p $output_dir
    srun -p gpu_p --qos=gpu_normal $constraint -o logs/$model_name-vars.o -e logs/$model_name-vars.e --nice=10000 -J $model_name-vars -c 4 --mem=64G --gres=gpu:1 --time=2-00:00:00 \
