...
sys.path.append("/home/icb/sergey.vilov/workspace/MLM/mpra_griesemer/utils/") 
...
data_dir = '/lustre/groups/epigenereg01/workspace/projects/vale/MLM/griesemer/'
...
if input_params.model in ("Species-aware","Species-agnostic","DNABERT","DNABERT-2","NT-MS-v2-500M"):

    X = np.load(data_dir + f"embeddings/{input_params.model}/embeddings.npy") #masked language model embeddings
    X = X[~flt]
...
def apply_regression(args):
    ...
    if input_params.regressor=='SVR':
        ...
        #y_pred_ridge = np.full_like(y_pred_svr, np.NaN)
        
    print('done')

    return list(zip(mpra_df.oligo_id.iloc[test_idx], y_pred))
...
all_res = pd.DataFrame(all_res, columns=['oligo_id','y_pred'])
