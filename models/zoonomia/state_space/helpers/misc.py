...
def get_chunks(seq_tokens, N_tokens_chunk, N_tokens_overlap, tokenizer_cls_token_id=None,
    ...
    assert [x for y in chunks[:-1] for x in y[:len(y)-N_tokens_overlap]]+[x for x in  chunks[-1][left_shift:]]==list(seq_tokens)
...
class LogPrinter():
    '''
    Print time string before the message
    use absolute time when reset=False,
    otherwise the time from class initialization
    '''
    ...
    def print(self, *args, **kwargs):
        if self.start_time is not None:
            now = time.monotonic()
            diff = round(now-self.start_time)
            now = f'[{diff//(60*60*24)}-{diff%(60*60*24)//(60*60)}:{diff%(60*60*24)%(60*60)//60}:{diff%(60*60*24)%(60*60)%60}]' ##%d-%H:%M:%S from self.init
        else:
            now = time.strftime("[%Y/%m/%d-%H:%M:%S]", time.localtime()) #absolute time
        builtins.print(now, *args, **kwargs)
        sys.stdout.flush()

def save_model_weights(model, tokenizer, optimizer, scheduler, scaler, meta, output_dir, checkpoint_name, save_at):
    '''
    ...
    os.makedirs(config_save_base, exist_ok=True)

    print(f'SAVING MODEL TO: {config_save_base}\n')

    if tokenizer is not None:
        tokenizer.save_pretrained(config_save_base)

    torch.save(model.state_dict(), config_save_base+'/model.pt') #save model weights
    ...
    torch.save(scaler.state_dict(), config_save_base+'/scaler.pt') #save optimizer weights

    with open(config_save_base+'/meta.json', "w") as f:
        json.dump(meta, f)

    for checkpoint_dir in os.listdir(output_dir):
        if checkpoint_name.split('_')[0]==checkpoint_dir.split('_')[0] and checkpoint_dir!=checkpoint_name and int(checkpoint_dir.split('_')[-1]) not in save_at:
            shutil.rmtree(os.path.join(output_dir, checkpoint_dir))
