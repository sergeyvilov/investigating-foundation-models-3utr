...
parser.add_argument("--step_size_up", help = "number of steps for lr ascending", type = int, default = 30000, required = False)
parser.add_argument("--step_size_down", help = "number of steps for lr descending", type = int, default = 470000, required = False)
...
scheduler = torch.optim.lr_scheduler.CyclicLR(optimizer, base_lr = 0, max_lr = input_params.max_lr,
                                             step_size_up = input_params.step_size_up,
                                             step_size_down = input_params.step_size_down,
...
def chkpt_callback(epoch, tot_steps, tokens_current_run, seqs_current_epoch, model, optimizer, scheduler, train_metrics):
    ...
    if (chkpt in input_params.save_at or -1 in input_params.save_at): #save model weights
        ...
        _ = os.system('cp ./DNABERT2/*.py ' + checkpoint_dir) #copy model architecture
