...
parser.add_argument("--step_size_up", help = "number of iterations for lr ascending", type = int, default = 30000, required = False)
parser.add_argument("--step_size_down", help = "number of iterations for lr descending", type = int, default = 470000, required = False)
...
if not input_params.test:
    ...
    for epoch in range(last_epoch+1, input_params.tot_epochs+1):
        ...
        if epoch in input_params.save_at or -1 in input_params.save_at: #save model weights
            ...
            checkpoint_dir = misc.save_model_weights(model, tokenizer, optimizer, scheduler, weights_dir, epoch, input_params.save_at)
            _ = os.system('cp ./DNABERT2/*.py ' + checkpoint_dir)
