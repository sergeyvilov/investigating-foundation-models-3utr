...
for masking in true ; do
    ...
    for ref_aware in false ; do
        ...
        srun python -u nt_inference.py --fasta $fasta --checkpoint_dir $checkpoint_dir --output_dir $output_dir  --ref_aware ${ref_aware} --N_folds $N_folds --fold $fold  --masking $masking --strand_bed $strand_bed  --whitelist $whitelist --central_window 2  > $output_dir/log_$fold 2>&1
        fi
