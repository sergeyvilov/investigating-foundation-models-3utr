...
#for model_name in stspace-3utr-2e stspace-spaw-3utr-2e  stspace-spaw-3utr-DNA stspace-3utr-DNA; do
for model_name in stspace-3utr-hs; do

    for mask_at_test in false ; do
    
        checkpoint_dir=$data_dir"models/zoonomia-3utr/$model_name/checkpoints"
    
        if [[ $model_name == *"DNA"* ]]; then
        ...
        --mask_at_test ${mask_at_test} --whitelist $whitelist --checkpoint_dir $checkpoint_dir --get_probs"
    
        mkdir -p $output_dir

        srun -p gpu_p --qos=gpu_normal -o ${output_dir}/log.o -e ${output_dir}/log.e --nice=10 -J $model_name-vars -c 4 --mem=64G --gres=gpu:1 --time=1-00:00:00 python -u main_parallel.py ${NN_PARAMETERS} &
