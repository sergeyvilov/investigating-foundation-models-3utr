...
if [ ! -f "$output_dir/predictions_$fold.pickle" ]; then
    srun python -u dna_bert_eval.py --fasta $fasta --checkpoint_dir $checkpoint_dir --output_dir $output_dir  --crop_lowercase --N_folds $N_folds --fold $fold --whitelist $whitelist --strand_bed $strand_bed > $output_dir/log_$fold 2>&1
fi
